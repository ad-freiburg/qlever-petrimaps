...
void Requestor::createGrid(util::geo::FBox pointBbox, util::geo::DBox lineBbox) {
  ...
  _lpgrid = petrimaps::Grid<util::geo::Point<uint8_t>, float>(GRID_SIZE, GRID_SIZE, fLineBbox);
  ...
  #pragma omp section
  {
    size_t i = 0;
    for (const auto& l : _objects) {
      if (l.first >= I_OFFSET && l.first < std::numeric_limits<ID_TYPE>::max()) {
        auto geomId = l.first - I_OFFSET;
