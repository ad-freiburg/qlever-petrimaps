...
#include "SPARQLCache.h"

using petrimaps::SPARQLCache;
...
SPARQLCache::SPARQLCache(const std::string& backendUrl) {
...
size_t SPARQLCache::writeCb(void* contents, size_t size, size_t nmemb, void* userp) {
  ...
  try {
    ...
    static_cast<SPARQLCache*>(userp)->_exceptionPtr = std::current_exception();
...
size_t SPARQLCache::writeCbIds(void* contents, size_t size, size_t nmemb, void* userp) {
  size_t realsize = size * nmemb;
  try {
    static_cast<SPARQLCache*>(userp)->parseIds(static_cast<const char*>(contents),
                                             realsize);
  } catch (...) {
    static_cast<SPARQLCache*>(userp)->_exceptionPtr = std::current_exception();
    return CURLE_WRITE_ERROR;
...
// _____________________________________________________________________________
void SPARQLCache::parse(const char* c, size_t size) {
...
// _____________________________________________________________________________
void SPARQLCache::parseIds(const char* c, size_t size) {
...
// _____________________________________________________________________________
void SPARQLCache::parseCount(const char* c, size_t size) {
  for (size_t i = 0; i < size; i++) {
    if (_raw.size() < 10000) _raw.push_back(c[i]);
    if (c[i] == '\n') _state = IN_ROW;
...
// _____________________________________________________________________________
size_t SPARQLCache::requestSize() {
  _state = IN_HEADER;
  _dangling.clear();
  ...
  if (_curl) {
    ...
    curl_easy_setopt(_curl, CURLOPT_WRITEFUNCTION, SPARQLCache::writeCbCount);
...
// _____________________________________________________________________________
void SPARQLCache::requestPart(size_t offset) {
  _state = IN_HEADER;
  _dangling.clear();
  _dangling.reserve(10000);
  _raw.clear();
  ...
  if (_curl) {
    ...
    curl_easy_setopt(_curl, CURLOPT_URL, qUrl.c_str());
    curl_easy_setopt(_curl, CURLOPT_WRITEFUNCTION, SPARQLCache::writeCb);
    curl_easy_setopt(_curl, CURLOPT_WRITEDATA, this);
    curl_easy_setopt(_curl, CURLOPT_ERRORBUFFER, errbuf);
    curl_easy_setopt(_curl, CURLOPT_SSL_VERIFYPEER, false);
    curl_easy_setopt(_curl, CURLOPT_SSL_VERIFYHOST, false);
...
void SPARQLCache::requestIds() {
  ...
  if (_curl) {
    ...
    curl_easy_setopt(_curl, CURLOPT_URL, qUrl.c_str());
    curl_easy_setopt(_curl, CURLOPT_WRITEFUNCTION, SPARQLCache::writeCbIds);
    curl_easy_setopt(_curl, CURLOPT_WRITEDATA, this);
    curl_easy_setopt(_curl, CURLOPT_SSL_VERIFYPEER, false);
    curl_easy_setopt(_curl, CURLOPT_SSL_VERIFYHOST, false);
    curl_easy_setopt(_curl, CURLOPT_HTTPHEADER, 0);
...
// _____________________________________________________________________________
std::string SPARQLCache::queryUrl(std::string query, size_t offset,
                                size_t limit) const {
  std::stringstream ss;
...
// _____________________________________________________________________________
util::geo::DLine SPARQLCache::parseLineString(const std::string& a,
                                            size_t p) const {
  util::geo::DLine line;
  line.reserve(2);
...
// _____________________________________________________________________________
util::geo::FPoint SPARQLCache::parsePoint(const std::string& a, size_t p) const {
...
SPARQLCache::getRelObjects(const std::vector<IdMapping>& ids) const {
  // (geom id, result row)
...
void SPARQLCache::insertLine(const util::geo::DLine& l, bool isArea) {
...
// _____________________________________________________________________________
std::string SPARQLCache::requestIndexHash() {
  CURLcode res;
  char errbuf[CURL_ERROR_SIZE];
  std::string response;
  ...
  if (_curl) {
    ...
    curl_easy_setopt(_curl, CURLOPT_WRITEFUNCTION, SPARQLCache::writeCbString);
